...
@Component({
  selector: 'app-new-code-set-mapping-modal',
  standalone: false,
  templateUrl: './new-code-set-mapping-modal.component.html',
  styleUrl: './new-code-set-mapping-modal.component.scss',
  providers: [EventService],
})
export class NewCodeSetMappingModalComponent implements OnInit, OnDestroy {
  // Signals
  ...
  public close() {
    this.eventService.publish('nf', 'close_new_code_set_mapping_modal', {
      action: 'close_new_code_set_mapping_modal',
      theme: this.theme(),
    });
  ...
  private subscribeToEvents() {
    ...
    this.eventSubs = this.eventStore.select('nf').subscribe((state: EventState) => {
      ...
      // Handle Open
      if (
        eventName === 'open_new_code_set_mapping_modal' ||
        payload?.action === 'open_new_code_set_mapping_modal'
      ) {
        ...
        this.mappingForm.reset({ status: 'Active' });
      }

      // Handle Close
      const closeEvents = ['close_new_code_set_mapping_modal', 'confirmation_save_confirmed'];
      const closeActions = ['close_new_code_set_mapping_modal', 'confirmation_save_confirmed'];
  ...
  public addMapping() {
    ...
    const code: CodeEntry = {
      codeValue: (this.mappingForm.get('codeValue')?.value || '').trim(),
      label: (this.mappingForm.get('label')?.value || '').trim(),
      description: (this.mappingForm.get('description')?.value || '').trim(),
      status: this.mappingForm.get('status')?.value,
    };

    this.eventService.publish('nf', 'code_set_mapping_added', {
      action: 'code_set_mapping_added',
      code: code,
      theme: this.theme(),
    });
