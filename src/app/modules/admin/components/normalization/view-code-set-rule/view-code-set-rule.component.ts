...
@Component({
  selector: 'app-view-code-set-rule',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './view-code-set-rule.component.html',
  styleUrl: './view-code-set-rule.component.scss',
})
export class ViewCodeSetRuleComponent implements OnInit, AfterViewInit, OnDestroy {
  @Input() ruleData: any;
  ...
  /**
   * Updates the breadcrumb to show the View Code Set Rule path
   */
  ...
  private subscribeToEvents(): void {
    this.eventSubs = this.eventStore.select('nf').subscribe((state: any) => {
      ...
      console.log(
        `view-code-set-rule.component.ts [subscribeEvents] - payload: ${JSON.stringify(payload)}`,
      );
  ...
  onCloseEdit(): void {
    ...
    // 3. Restore breadcrumb to "View Code Set Rule" state
    this.updateBreadcrumb();
