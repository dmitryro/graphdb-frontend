...
@Component({
  selector: 'app-edit-mapping-rule',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './edit-mapping-rule.component.html',
  styleUrl: './edit-mapping-rule.component.scss',
})
export class EditMappingRuleComponent implements OnInit, AfterViewInit, OnDestroy {
  @Input() ruleData: any;
  ...
  private subscribeToEditEvents(): void {
    this.eventSubs = this.eventStore.select('nf').subscribe((state: any) => {
      ...
      if (eventName === 'breadcrumb_navigate' && payload?.target === 'VIEW_MAPPING_RULE') {
        this.onCancel();
      }
  ...
  private updateBreadcrumb(): void {
    const breadcrumbPath = [
      ...
      { label: 'View Mapping Rule', target: 'VIEW_MAPPING_RULE' },
      { label: 'Edit Mapping Rule', active: true },
    ];
