...
@Component({
  selector: 'app-edit-code-set-rule',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './edit-code-set-rule.component.html',
  styleUrl: './edit-code-set-rule.component.scss',
})
export class EditCodeSetRuleComponent implements OnInit, AfterViewInit, OnDestroy {
  @Input() ruleData: any;
  ...
  private subscribeToEditEvents(): void {
    this.eventSubs = this.eventStore.select('nf').subscribe((state: any) => {
      ...
      if (eventName === 'breadcrumb_navigate' && payload?.target === 'VIEW_CODE_SET_RULE') {
        this.onCancel();
      }
  ...
  private updateBreadcrumb(): void {
    const breadcrumbPath = [
      ...
      { label: 'View Code Set Rule', target: 'VIEW_CODE_SET_RULE' },
      { label: 'Edit Code Set Rule', active: true },
    ];
  ...
  onCancel(): void {
    ...
    const breadcrumbPath = [
      { label: 'Normalization', target: 'ROOT' },
      { label: 'Rules', target: 'TAB_RULES' },
      { label: 'View Code Set Rule', active: true },
    ];
    this.eventService.publish('nf', 'update_breadcrumb', { path: breadcrumbPath });
