...
@Component({
  selector: 'app-edit-mixed-mapping',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './edit-mixed-mapping.component.html',
  styleUrl: './edit-mixed-mapping.component.scss',
})
export class EditMixedMappingComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
    ...
    if (!this.mappingData) {
      this.eventStore
        ...
        .subscribe(state => {
          ...
          if (lastEvent?.event === 'open_edit_mixed_mapping' && lastEvent?.payload?.mappingData) {
  ...
  private subscribeToEvents(): void {
    this.eventSubs = this.eventStore.select('nf').subscribe((state: any) => {
      ...
      if (eventName === 'open_edit_mixed_mapping' && payload?.mappingData) {
      ...
      // Handle breadcrumb navigation back to view
      if (eventName === 'breadcrumb_navigate' && payload?.target === 'VIEW_MIXED_MAPPING') {
        this.onCancel();
      }
      ...
      if (eventName === 'confirmation_discard_confirmed') {
        this.executeActualDiscard();
      }
    });
  }
  ...
  private updateBreadcrumb(): void {
    const breadcrumbPath = [
      ...
      { label: 'View Mixed Mapping', target: 'VIEW_MIXED_MAPPING' },
      { label: 'Edit Mixed Mapping', target: 'EDIT_MIXED_MAPPING', active: true },
    ];
  ...
  private executeActualSave(): void {
    this.eventService.publish('nf', 'mixed_mapping_updated', {
      action: 'mixed_mapping_updated',
  ...
  private executeActualDiscard(): void {
    this.exitAndReturnToView();
  }
  ...
  private exitAndReturnToView(): void {
    ...
    // Update breadcrumb to return to View Mode
    const backToViewPath = [
      { label: 'Normalization', target: 'ROOT' },
      { label: 'Mappings', target: 'TAB_MAPPINGS' },
      { label: 'View Mixed Mapping', target: 'VIEW_MIXED_MAPPING', active: true },
    ];
  ...
  onValueChange(row: MappingRow): void {
    ...
    row.isModified = rowHasAnyChange;
