...
describe("AuthService", () => {
  ...
  it("should log in failed", () => {
    authService
      ...
      .subscribe((isLogin) => expect(isLogin).toBeFalse());
  ...
  it("should log in successful and get user info", () => {
    user$
      .pipe(skip(1))
      .subscribe((currentUser) => expect(currentUser.id).toEqual(user.id));
    authService
      .login(email, "password", false)
      .subscribe((isLogin) => expect(isLogin).toBeTrue());
  ...
  it("should log out successful when user is login", () => {
    ...
    user$
      .pipe(skip(1))
      .subscribe((currentUser) => expect(currentUser.id).toBeUndefined());
    authService.logout().subscribe();
  ...
  it("should refresh token when access_token is valid", fakeAsync(() => {
    ...
    const match = (req: HttpRequest<any>) =>
      req.url === "/auth/refresh" && !req.body.refresh_token;
  ...
  it("should refresh token when access_token is invalid and refresh_token is valid", fakeAsync(() => {
    tokenService.set(
      Object.assign({ expires_in: 5, refresh_token: "foo" }, token),
    );
    const match = (req: HttpRequest<any>) =>
